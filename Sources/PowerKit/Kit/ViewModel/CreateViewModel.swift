...
internal class CreateViewModel: NSObject {
    ...
    func addNewTo(_ settings: [PowerItemModel], header: PowerCells? = nil, newItems: [PowerCells], section: Int, at: Int? = nil) {
        guard settings.isEmpty == false else { initFatelError(); return }
        addNewItems(settings: settings, items: newItems, headerItem: header, forSection: section, at: at)
    }
...
extension CreateViewModel {
    ...
    private func addNewItems(settings: [PowerItemModel], items: [PowerCells], headerItem: PowerCells?, forSection: Int, at: Int? = nil) {
        ...
        items.forEach { appendOrInsertNewItem(at: at, powerItemModel: model, newItem: $0, forSection: forSection) }
    }
    ...
    private func createNewItem(at: Int?, powerItemModel: PowerItemModel, newItem: PowerCells) {
